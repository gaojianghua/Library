...
<template>
	<view style="height: 100%;">
		...
		<!-- ************************** -->
		<!-- 封面 -->
		<view class="cover position-fixed top-0 left-0 d-flex flex-column a-center h-100 w-100"
			:class="`container${background - 1}`" :style="{zIndex: 201, transform: `translate${cover.pageTranslate[turnType]}`, transition: `transform ${showAnimation?turnPageTime:0}s`,
			...
			<image style="border-radius: 16rpx; margin-top: 100rpx;" :src="detail.picx"></image>
			<view class="mt-2 font-weight" :style="{color:background == 2 ? colorList[1] : colorList[0]}">
				{{detail.name}}
			</view>
		...
		<view class="container" :class="`container${background - 1}`" :style="{zIndex: 102, transform: `translate${prePage.pageTranslate[turnType]}`, transition: `transform ${showAnimation?turnPageTime:0}s`,
			...
			<view class="" :style="{height: $store.state.statusHeight + 'px'}"></view>
			<!-- 章节名 -->
			<view class="chapter text-ellipsis1">
				{{prePage.chapterName}}
			</view>
			...
			<view class="content">
				...
				<view style="display:flex;flex-flow: column;justify-content: center;align-items: center;height: 100%;"
					v-else>
					<span class="main-text-color">{{$t('当前为VIP章节，请开通VIP服务后继续阅读')}}</span>
					<view class="mt-5 text-white"
						style="padding: 5rpx 40rpx;background-color: #6F43FE;border-radius: 50rpx;">
						{{$t('去开通')}}
					</view>
				</view>
		...
		<!-- 菜单层 -->
		<view class="menu w-100 h-100" v-if="menuShow" @touchend="closeMenu">
			<!-- 菜单层包含返回按钮的上半部分 -->
			<view class="menu-top" :style="{
				height: `${statusBarHeight + 40}px`, 
				top: itemShow ? 0 : `-100%`,
				background: backColor
			}" @touchend.stop>
				<view :style="{height: `${statusBarHeight}px`}"></view>
				<view class="head d-flex a-center px-2">
					<u-icon name="arrow-left" size="20" :color="background != 1 ? bgList[1].color : bgList[0].color"
						@click="back"></u-icon>
					<view class="show-chapter text-center ml-2 text-ellipsis1" :style="{
						color: background != 1 ? bgList[1].color : bgList[0].color
					}" v-if="progressTouched">{{directoryList[chapterProgress].name}}</view>
					<view class="show-chapter text-center ml-2 text-ellipsis1" :style="{
						color: background != 1 ? bgList[1].color : bgList[0].color
					 }" v-else>{{curChapter.chapterName}}</view>
					<view class="ml-auto" @click="shareMethods">
						<u-icon name="share-square" size="26" color="#6F43FE"></u-icon>
					</view>
					<view class="d-flex a-center flex-shrink j-center ml-2"
						@click="detail.fav == 0 ? addCollect() : collShow = true">
						<u-icon v-if="detail.fav == 0" name="heart" color="#6F43FE" size="24"></u-icon>
						<u-icon v-else name="heart-fill" color="#6F43FE" size="24"></u-icon>
					</view>
				</view>
			</view>
			<!-- 菜单主体 -->
			<view class="menu-bottom pt-2" :style="{bottom: itemShow ? 0 : '-100%',
			background: backColor
			}" @touchend.stop>
				<!-- 章节进度条 -->
				<view class="progress-box">
					<u-icon name="arrow-left" size="24" :color="background != 1 ? bgList[1].color : bgList[0].color"
						@click="goPreChapter"></u-icon>
					<view style="flex: 1;height: 100%;padding: 8px 0;">
						<slider :blockColor="background != 1 ? bgList[1].color : bgList[0].color"
							:value="curChapter.chapterIndex" activeColor="#6F43FE" :block-size="20"
							:max="directoryList.length - 1" @changing="slideChanging" @change="slideChange" />
					</view>
					<u-icon name="arrow-right" size="24" :color="background != 1 ? bgList[1].color : bgList[0].color"
						@click="goNextChapter"></u-icon>
				</view>
				<view class="items-box">
					...
					<view class="item-box" v-if="background != 2" @click="changeBackground(2)">
						<u-icon :name="`/static/img/common/${background == 1 ? 'hei' : 'hei-all'}.svg`"
							size="28"></u-icon>
						<text :style="{
							color: background != 1 ? bgList[1].color : bgList[0].color
						}">{{$t('夜间')}}</text>
					...
					<view class="item-box" v-else @click="changeBackground(1)">
						<u-icon :name="`/static/img/common/bai.svg`" size="28"></u-icon>
						<text :style="{
							color: background != 1 ? bgList[1].color : bgList[0].color
						}">{{$t('日间')}}</text>
					</view>
			...
			<view class="setting w-100 d-flex flex-column j-around pb-2" :style="{bottom: settingShow ? 0 : `-100%`,
			background: '#fff',
			color: '#111'
			}" @touchend.stop>
				<view class="d-flex position-relative a-center j-center text-white main-bg-color set-title">
					{{$t('阅读设置')}}
					<u-icon @click="closeMenu" style="top: 50%; right: 20rpx;transform: translateY(-50%);" name="close"
						color="#fff" size="16" class="position-absolute"></u-icon>
				</view>
				<view class="item px-2 mt-2">
					<view class="item-name mr-2">{{$t('字号')}}</view>
					...
					<view class="font-weight font-md mr-3">
						{{fontSize}}
					</view>
					<view class="icon" @click="smallSize" v-if="fontSize>minFontSize">A-</view>
					<view class="icon" style="color: #666;border: #666 solid 1px;" v-else>A-</view>
				</view>
				<view class="item px-2">
					<view class="item-name mr-2">{{$t('间距')}}</view>
					<view class="type-setting d-flex a-center flex-column j-around"
						:class="{active: lineHeight === 1.5}" @click="changeLineHeight(1.5)">
						<view class="line" :class="{lineActive: lineHeight === 1.5}" v-for="i in 5" :key="i"></view>
					</view>
					<view class="type-setting d-flex a-center flex-column j-around" :class="{active: lineHeight === 2}"
						@click="changeLineHeight(2)">
						<view class="line" :class="{lineActive: lineHeight === 2}" v-for="i in 4" :key="i"></view>
					</view>
					<view class="type-setting d-flex a-center flex-column j-around"
						:class="{active: lineHeight === 2.5}" @click="changeLineHeight(2.5)">
						<view class="line" :class="{lineActive: lineHeight === 2.5}" v-for="i in 3" :key="i"></view>
					</view>
				</view>
				<view class="item px-2">
					<view class="item-name mr-2">{{$t('模式')}}</view>
					<view class="double px-2" :class="{active: turnType === 0}" @click="changeTurnType(0)">{{$t('覆盖')}}
					</view>
					<view class="double px-2" :class="{active: turnType === 1}" @click="changeTurnType(1)">{{$t('平移')}}
					</view>
					<view class="double px-2" :class="{active: turnType === 2}" @click="changeTurnType(2)">{{$t('滚动')}}
					</view>
				</view>
				<view class="item px-2">
					<view class="item-name mr-2">{{$t('主题')}}</view>
					<view class="icon" style="background-color: #fff;" :class="{active: background === 1}"
						@click="changeBackground(1)"></view>
					<view class="icon" style="background-color: #111;" :class="{active: background === 2}"
						@click="changeBackground(2)"></view>
					<view class="icon" style="background-color: #E8E2CC;" :class="{active: background === 3}"
						@click="changeBackground(3)"></view>
					<view class="icon" style="background-color: #BECEBF;" :class="{active: background === 4}"
						@click="changeBackground(4)"></view>
					<view class="icon" style="background-color: #DCE2F1;" :class="{active: background === 5}"
						@click="changeBackground(5)"></view>
					<view class="icon" style="background-color: #F1DFC2;" :class="{active: background === 6}"
						@click="changeBackground(6)"></view>
				</view>
			</view>

			<!-- 目录层 -->
			<view class="directory" :class="`container${background - 1}`"
				v-if="directoryShowBefore" :style="{left: directoryShow ? 0 : '-100%',
				color: background != 1 ? bgList[1].color : bgList[0].color,
				background: backColor
				}" @touchend.stop>
				<view class="nation-title d-flex a-center px-2 j-sb"
					...
					<view class="text-ellipsis1">
						{{detail.name}}({{directoryList.length}})
					</view>
					<view class="d-flex ml-2 flex-shrink a-center j-center">
						<view class="" :style="{color: isSort == 0 ? '#6F43FE' : '#666'}" @click="setIsSort(0)">
							{{$t('正序')}}
						</view>
						<view class="px-1" style="color: #6F43FE;">
							|
						</view>
						<view class="" :style="{color: isSort == 1 ? '#6F43FE' : '#666'}" @click="setIsSort(1)">
							{{$t('倒序')}}
						</view>
					</view>
				</view>
				...
				<virtual-list :items="directoryList" :size="40" :remain="16" :active="curChapter.chapterIndex"
					:scrollHeight="windowHeight - 80">
					<template v-slot="{item,active}">
						<view class="directory-listItem mt-2" :class="{active: item.index == active}"
							@click="goToChapter(item.index)">
							<view v-if="item.vip!=='0'" class="isvip line-h font-weight mr-2">
								VIP
							</view>
							<view class="name line-h-sm">
								{{item.name}}
							</view>
		...
		<!-- 弹框 -->
		<m-modal :show="modalShow" i18n title="温馨提示" confirmName="加入书架" @cancel="cancel" zIndex="11"
			@confirm="addCollect('s')">
	...
	export default {
		...
		methods: {
			...
			async addCollect(e) {
				...
				let { code } = await addNovelShelf(result)
				if (code == 1) {
					this.detail.fav = 1
					uni.showToast({
						icon: 'none',
						...
						title: this.$t("收藏成功"),
						success: () => {
							e && this.$tools.Navigate.navigateBack()
						}
					});
				}
			},
			...
			async delCollect() {
				...
				let {code} = await delNovelShelf(result)
				if (code == 1) {
					this.detail.fav = 0
					uni.showToast({
						icon: 'none',
			...
			async getThreeChapter(index) {
				if (this.query.isResume) {
					this.directoryList.forEach((item, i) => {
		...
		computed: {
			...
			backColor() {
				let num1 = this.background - 1
				let num2 = this.background - 2
				let str = this.background == 1 ? this.bgList[this.background].color : (this.background == 2 ? this.bgList[num2].color :
					this.bgList[num1].color)
				return str
			}
		}
	}
		...
		.setting {
			...
			.item {
				...
				.type-setting {
					...
					.lineActive {
						width: 100%;
						border-bottom: #764BFF solid 1px;
					}
				}

				.active {
					color: #764BFF;
					border: #764BFF solid 1px;
				}
			}
		}
	}
