...
import { getIPCity } from '@/utils/request/api/get.js'
import w_md5 from "@/js_sdk/zww-md5/w_md5.js"

export const requestData = (query = {}) => {
	let deviceId = store.state.deviceId
	// let facility = store.state.facility
	...
	if(store.state.token) {
		oldObj.user_id = store.state.userinfo.id
		oldObj.user_token = store.state.token
	}
	
	let sortObj = objSort(oldObj)
	let param = assemblyParam(sortObj)
	
	let sign = w_md5.hex_md5_32Upper(param + 'fm_zsec456frvvc123cxew')
	return {
		...sortObj,
		sign
	}
...
// 参数组装
const assemblyParam = (obj) => {
	let str = ''
	Object.keys(obj).forEach((item, i) => {
		i == 0 ? (str = `${item}=${obj[item]}`) : (str = str + `&${item}=${obj[item]}`)
	})
	return str
}

// 根据IP获取国家信息
export const getIPCitys = async () => {
	let result = requestData({})
	let {ip} = await getIPCity(result)
	getIPNation(ip)
}
// 根据IP获取国家信息
const getIPNation = (ip) => {
	uni.request({
		...
		success: (res) => {
			store.commit('updateNationInfo', res.data)
		},
	})
}
...
// 			console.log(res)
// 			uni.setStorageSync('facility', res.platform)
// 			store.commit('updateFacility', res.platform)
// 			uni.setStorageSync('deviceid', res.deviceId)
// 			store.commit('updateDeviceid', res.deviceId)
// 		}
// 	});
// 	// #ifdef APP-PLUS
// 	if(store.state.facility=='ios'){
// 		plus.device.getInfo({
// 			success: function(e) {
// 				console.log(e);
// 				store.commit('updateDeviceid', e.uuid)
// 			},
// 			fail: function(e) {
// 				console.log(e);
// 			}
...
// 			success: function(e) {
// 				console.log(e);
// 				store.commit('updateDeviceid', e.aaid)
// 			},
// 			fail: function(e) {
// 				console.log(e);
// 				plus.device.getInfo({
// 					success: (res) => {
// 						store.commit('updateDeviceid', res.uuid)
// 					}
// 				})
// 			}
// 		});
