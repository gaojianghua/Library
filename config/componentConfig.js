...
export default {
	...
	getServerNo: (version, isPrompt = false, callback) => {
		...
		let result = requestData({
			facility: httpData.platform,
			version: httpData.versionName
		})
		console.log(result)
		get('/index.php/appv1/init/update', result).then(res => {
			let data = {}
			if (res.code == 1) {
				data = {
					versionCode: res.version.replace(/\./g, ''),
					versionName: res.version,
					downloadUrl: res.downurl,
					updateType: res.force == 1 ? 'forcibly' : 'solicit',
					versionInfo: res.text
				}
			}
			/* data的数据说明
			 * | 参数名称        | 一定返回     | 类型        | 描述
			...
			if (data && data.downloadUrl) {
				...
				if (data.versionCode <= appUpdateInfo.versionCode &&  appUpdateInfo.expired > new Date().getTime() && !httpData.setupPage) {
					return
				}
				if (data.updateType) {
